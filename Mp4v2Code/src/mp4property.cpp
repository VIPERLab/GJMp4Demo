...
void MP4StringProperty::Read( MP4File* pFile, uint32_t index )
{
    ...
    for( uint32_t i = begin; i < max; i++ ) {
        ...
        else if( m_fixedLengthSet ) {
            // gfinlayson
            // Handle the case where a fixed length of 0 was set, in as defensive a way
            // as possible
            MP4Free( value );
...
void MP4BytesProperty::Read(MP4File* pFile, uint32_t index)
{
    ...
    MP4Free(m_values[index]);
    m_valueStart[index] = pFile->GetPosition();
